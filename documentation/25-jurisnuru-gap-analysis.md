...
| Reports | 14 | **REAL** | `firm_overview()` aggregate over matters, documents, filing records, fees and receipts; `tests/validate_wakilios_api.py` |
...
| Firm dashboard: payable/paid/balance and per-station counts | — | **PARTIAL** | `firm_overview()` returns totals, balance and per-station counts; rendered as a list rather than the portal's tiles |
...
5. **Firm dashboard presentation**, over the `firm_overview()` aggregate that now exists.
